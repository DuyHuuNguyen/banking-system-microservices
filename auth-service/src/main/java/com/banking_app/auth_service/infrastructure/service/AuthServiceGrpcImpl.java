...
import com.example.server.grpc.AuthProto;
...
//    private final ExecutorService executor = Executors.newSingleThreadExecutor();
    ...
    @Override
    public void parseToken(AccessTokenRequest request, StreamObserver<AuthResponse> responseObserver)  {
        String accessToken = request.getAccessToken();
        var isValidateToken = this.jwtService.validateToken(accessToken);
        if (!isValidateToken) responseObserver.onError(new JwtException(ErrorCode.JWT_INVALID.getMessage()));
        ...
        accountService.findByPersonalIdentificationNumber(personalIdentifyInformation)
...
//    private Future<Mono<AuthResponse>> authResponseFeature(String personalId){
//        return executor.submit(() -> {
//            return   this.accountService
//                    .findByPersonalIdentificationNumber(personalId)
//                    .switchIfEmpty(Mono.error(new EntityNotFoundException(ErrorCode.ACCOUNT_NOT_FOUND)))
//                    .flatMap(this::buildAuthResponse);
//        });
//    }
    ...
    private Mono<AuthResponse> buildAuthResponse(Account account){
        return this.roleService
                ...
                .flatMap(
                        roles ->
                                Mono.just( AuthResponse.newBuilder()
